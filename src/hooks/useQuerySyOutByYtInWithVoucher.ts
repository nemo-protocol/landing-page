...
export default function useQuerySyOutByYtInWithVoucher(
  ...
  const mutation = useMutation({
    mutationFn: async (inputAmount: string) => {
      ...
      if (!coinConfig) throw new Error("Please select a pool")

      const debugInfo: DebugInfo = {
        moveCall: {
          target: `${coinConfig.nemoContractId}::market::get_sy_out_for_exact_yt_in_with_price_voucher`,
          arguments: [
            { name: "net_yt_in", value: inputAmount },
