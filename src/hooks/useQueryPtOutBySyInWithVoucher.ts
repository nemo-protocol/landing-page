...
export default function useQueryPtOutBySyInWithVoucher(coinConfig?: CoinConfig, address?: string) {
  ...
  return useMutation({
    mutationFn: async (inputAmount: string) => {
      ...
      if (!result?.results?.[1]?.returnValues?.[0]) {
        throw new Error("Failed to get PT amount")
      }
