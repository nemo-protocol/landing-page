          ...
          {list?.map((item) => (
            ...
            >
              ...
              <TableCell className="!px-2" align="center">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="bg-[#0F60FF] text-white rounded-xl px-4 py-2 flex items-center justify-between w-[192px] h-[52px] cursor-pointer border border-transparent hover:border-white"
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/market/detail/${item.coinType}/${item.maturity}/swap/yt`);
                      }}>
                      <span>YT</span>
                  ...
                  <TooltipContent side="top" className="bg-black text-white border-none">
                    <p>Leveraged APY - Earn leveraged staking rewards</p>
                  </TooltipContent>
                </Tooltip>
              </TableCell>
              <TableCell className="!px-2" align="center">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="bg-[#2DF5DD] text-black rounded-xl px-4 py-2 w-[200px] h-[52px] flex items-center justify-between cursor-pointer border border-transparent hover:border-white"
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/market/detail/${item.coinType}/${item.maturity}/swap/pt`);
                      }}>
                      <span>PT</span>
                      <div className="flex flex-col items-end">
                        <span>
                  ...
                  <TooltipContent side="top" className="bg-black text-white border-none">
                    <p>Fixed APY - Lock in a guaranteed fixed rate</p>
                  </TooltipContent>
                </Tooltip>
              </TableCell>
              <TableCell className="!px-2" align="right">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="bg-[#62CAFF] text-black rounded-xl pl-4 pr-2 py-2 w-[200px] h-[52px] flex items-center justify-between cursor-pointer border border-transparent hover:border-white"
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <TooltipContent side="top" className="bg-[#1B202A] text-white border-none p-4 relative rounded-xl w-[312px]">
                    <div className="flex flex-col gap-4">
                      <div className="flex flex-row items-center justify-between gap-4">
                        <span className="text-sm">Scaled</span>
                        <span className="font-mono text-sm">
                          {item.ptApy && item.underlyingApy 
                            ? `${formatLargeNumber(Number(item.ptApy) + Number(item.underlyingApy), 6)}%` 
                            : "--"}
                        </span>
                      </div>
                      <div className="relative flex flex-col gap-2">
                        <div className="relative flex flex-row gap-2">
                          ...
                          <div className="flex flex-1 flex-row items-start justify-between gap-4">
                            ...
                            <span className="font-mono text-xs">{item.ptApy ? `${formatLargeNumber(item.ptApy, 6)}%` : "--"}</span>
                          </div>
                        </div>
                        <div className="relative flex flex-row gap-2">
                          <div className="-mt-1 h-3 w-3 rounded-bl-md border-b border-l border-[#41517A]"></div>
                          <div className="flex flex-1 flex-row items-start justify-between gap-4">
                            ...
                            <span className="font-mono text-xs">{item.underlyingApy ? `${formatLargeNumber(item.underlyingApy, 6)}%` : "--"}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col gap-2">
                        <div className="flex flex-row items-center justify-between gap-4">
                          <span className="text-sm">Incentive APY</span>
